...
class ViewController: UIViewController,
  ...
  func applyNumKartsSetting(_ settings: [String : Any]) {
      ...
      guard let numKarts = settings["numKarts"] as? Int else{
          ...
          assertionFailure(
            "Expected 'numKarts' setting to be an Int, but found something else instead."
          )
          return
      }
  ...
  func applySpeedMultiplierSetting(_ settings: [String : Any]) {
    ...
    self.speedMultiplier = Double(speedMultiplier) // Setting it to Double and assign it to the Kart
  }
}
